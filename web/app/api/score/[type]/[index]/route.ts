...
export async function GET(
    ...
    return NextResponse.json(
      {
        error: "not found",
        key,
        hint: "scoring has not run yet for this index. scoring is the Phase 20B pass; Phase 20A only extracts claims.",
      },
      { status: 404 }
    );
  }
  try {
    ...
    return NextResponse.json(doc, {
      headers: {
        // Score documents are written once per scoring pass and don't
        // change after. Cache them on the edge.
        "Cache-Control":
